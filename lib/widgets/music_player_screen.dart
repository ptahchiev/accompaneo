...
class MusicPlayerScreen extends StatefulWidget {
  ...
  const MusicPlayerScreen({required this.musicData});
...
class _MusicPlayerScreenState extends State<MusicPlayerScreen>
  ...
  void _startSegmentTimer() {
    ...
    _controller.duration = Duration(
      milliseconds: segmentDuration == 0 ? 100 : (segmentDuration * 1000).toInt(),
    );
...
//TODO - add song finished logic
            ...
            if (orientation == Orientation.portrait) {
              return Column(
                children: [
                  ...
                  Container(height: Dimensions.smallMargin),
  ...
  Widget _nextChordWidget(Bar bar) {
    ...
    //GuitarChord? chord = stringToChord(nextChord?.name ?? '');
    var instrument = GuitarChordLibrary.instrument(InstrumentType.guitar);
    Chord chord = instrument.getChordsByKey(nextChord?.name ?? '', false)![0];
  ...
  Widget _segmentWidget({
    ...
    List<Event> chords =
        bar.events.where((e) => e.type == EventType.chord).toList();
    double padding = 16 ?? Dimensions.padding;
